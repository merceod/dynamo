...
<Tabs>
  <Tab title="Codex">
    ...
    The runner pins `@agentclientprotocol/codex-acp`, uses the returned Codex thread ID as its ACP session ID, and closes the ACP process when stdin closes. It passes the child a minimal environment containing runtime, temporary-directory, and locale values; the selected Dynamo credential; and fresh Codex endpoint/model configuration. Other parent credentials and an ambient `CODEX_CONFIG` are not inherited.
    ...
    For ThunderAgent, use the explicitly lifecycle-enabled command:
    ...
    After the ACP process closes, the runner attempts exactly one terminal request for the created Codex thread on normal completion, turn failure, or interruption. It skips finalization when no nonempty thread ID was created, and a terminal failure cannot silently pass the run. See the [Codex well-lit path](https://github.com/ai-dynamo/dynamo/blob/main/examples/agent_harnesses/codex/README.md) for dependency pins, verification, known limitations, and cleanup steps.
