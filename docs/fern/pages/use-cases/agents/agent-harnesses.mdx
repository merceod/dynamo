...
<Tabs>
  ...
  <Tab title="Omnigent">
    ...
    Omnigent is an open-source meta-harness that can run Codex against an OpenAI-compatible gateway. Install a supported Omnigent release, then use `omnigent setup` to add a Gateway credential for Codex. Set its base URL to the Dynamo recipe endpoint with `/v1` appended and use the recipe's API key, or any nonempty value for an unauthenticated local endpoint.
    ...
    Codex sends streaming `/v1/responses` requests with `thread-id`, which Dynamo normalizes as the session ID. Treat this as a stock-Dynamo compatibility path: Omnigent does not currently send the ThunderAgent terminal lifecycle header, and a one-command run does not prove persistent main-thread reuse. Run it in a dedicated environment if cleanup requires `omnigent stop`, because that command stops all local Omnigent processes. See the [Omnigent well-lit path](https://github.com/ai-dynamo/dynamo/blob/main/examples/agent_harnesses/omnigent/README.md) for verification and cleanup boundaries.
